...
## Merge policy — DECIDED 2026-08-21

**Agreed: slices merge into `redesign/00-foundation` (PR #5) — the stack
base — and NOTHING merges to `main` until the whole redesign is published
and deployed to prod in one go.**

- When a slice PR is accepted, merge it into the `redesign/00-foundation`
  branch (or its successor state on the stack), never into `main`.
- `main` stays production-safe: the deploy workflow (`deploy.yaml` on main
  pushes) must not fire from redesign work. Production keeps the old design
  until we deliberately ship the redesign.
- Practical flow: as slices merge down into 00, the remaining stack
  rebases onto the updated 00; the *integrated* state is always reviewable
  on 00's preview URL (`redesign-00-foundation.dev.michalvanko.dev`).
- Shipping = one deliberate action at the end: merge `redesign/00-foundation`
  → `main` (squash or merge, then delete slice branches), which triggers the
  prod deploy. Do it only when the design is signed off.
