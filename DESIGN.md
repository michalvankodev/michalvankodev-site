...
- Cover side plate (~≤500px, chapter-intro style) sits beside the article
  ...
  frontispiece.
- Page containers: the `page-gutter` class (input.css) is the ONE source
  of horizontal gutter; content blocks carry no horizontal padding of
  their own (a per-block `px-4` era is gone — it
